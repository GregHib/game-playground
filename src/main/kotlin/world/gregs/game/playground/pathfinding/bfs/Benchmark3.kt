...
object Benchmark3 {
    ...
    object distances {
        ...
        fun index(x: Int, y: Int) = ((x shl 6) or y) shl 2

        operator fun set(x: Int, y: Int, value: Int) {
            unsafe.putInt(address + index(x, y), value)
        }
        operator fun get(x: Int, y: Int): Int {
            return unsafe.getInt(address + index(x, y))
        }

        val fillAddress = unsafe.allocateMemory(bytes)

        init {
            for (x in 0..columns-1) for (y in 0..rows-1) unsafe.putInt(fillAddress + index(x, y), fillF)
            fill()
        }
    ...
    object queue {
        const val bytes = Benchmark2.colbyrows * 8L
        val address = Benchmark2.unsafe.allocateMemory(bytes)

        fun index(i: Int) = i shl 2

        operator fun set(i: Int, x: Int, y: Int) = Benchmark2.unsafe.putLong(address + index(i), xy(x, y))
        operator fun get(i: Int) = Benchmark2.unsafe.getLong(address + index(i))
        ...
        val fillAddress = Benchmark2.unsafe.allocateMemory(bytes)

        init {
            val negative1 = xy(-1, -1)
            for (i in 0..Benchmark2.colbyrows -1) {
                Benchmark2.unsafe.putLong(fillAddress + index(i), negative1)
            }
        }

        fun fill() = Benchmark2.unsafe.copyMemory(fillAddress, address, bytes)
    }
    ...
    fun check(parentX: Int, parentY: Int, x: Int, y: Int) {
        val px = parentX + x
        val py = parentY + y
        if (!collision[px, py] && distances[px, py] == -1) {
            distances[px, py] = distances[parentX, parentY] + 1
