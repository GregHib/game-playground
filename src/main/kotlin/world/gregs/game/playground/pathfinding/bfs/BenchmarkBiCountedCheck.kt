...
object BenchmarkBiCountedCheck {
    @JvmStatic
    fun main(args: Array<String>) {
        ...
        fun check(parentX: Int, parentY: Int, x: Int, y: Int, type: Int): Boolean {
            if (collision.getOrNull(parentX + x)?.getOrNull(parentY + y) == false) {
                if (getVisit(frontier[index(parentX + x, parentY + y)]) == visit + if (type == 1) 0 else 1) {
                    return true
                } else if (getVisit(frontier[index(parentX + x, parentY + y)]) != visit + type) {
        ...
        fun bfs(): Long {
            return measureNanoTime {
                ...
                while (readIndex < writeIndex) {
                    ...
                    if(check(parentX, parentY, -1, 0, type))
                        break
                    if(check(parentX, parentY, 1, 0, type))
                        break
                    if(check(parentX, parentY, 0, -1, type))
                        break
                    if(check(parentX, parentY, 0, 1, type))
                        break
                    if(check(parentX, parentY, -1, -1, type))
                        break
                    if(check(parentX, parentY, 1, -1, type))
                        break
                    if(check(parentX, parentY, -1, 1, type))
...
//    randomise()
        ...
        println(visit)
//        BFS took 2584059900 avg 258405
    }
}